...
class Page {
    ...
    protected $coolPages   ;
    // 分页显示定制
    protected $config  =    array('header'=>'条记录','prev'=>'上一页','next'=>'下一页','first'=>'第一页','last'=>'最后一页','theme'=>'<li class="disabled"><span> %totalRow% %header% %nowPage%/%totalPage% 页</span></li>%upPage% %downPage% %first%  %prePage%  %linkPage%  %nextPage% %end%');
    // 默认分页变量名
    protected $varPage;
    ...
    public function show() {
        ...
        if($this->url){
            ...
            //$url            =   U('',$parameter);
            $url= U(GROUP_NAME.'/Index/logininfo',$parameter);//第99行，重新写url 
        }
        //上下翻页字符串
